...
c Version for FLEUR (FleurMaX Release 5.1 version 34)
c
c ---------------------------------------------------
c
...
# include "cppmacro.h"
      ...
      character(*), parameter :: dftlong    = 'FleurMaX Release 5.1 version 34'
      character(*), parameter :: dftshort   = 'fleurR5.1'
...
c ----------------------------------
      ...
      call hdf_rdwr  (Hfile,'atoms/nlo',     0,nlo)  ; nlod = maxval(nlo)
...
c ----------------------------------
 ...
 2    call fclose(iunit_s)
      ...
      write(6,'(A/)') '  Also, do not forget to set spex="2" and numbands="(max number of bands)" '//
     &                'if Fleur should prepare input data for Spex.'
      ...
      integer                  :: i,kgv
      scale = kgv(nkpt3,3)
      call write_kpt_xml('kpts.xml','spex-path','path',qpt,nqpt,[(0d0,i=1,nqpt)],scale,label)
      write(6,'(A)')  'File kpts.xml complemented with Spex q-point path.'
      write(6,'(A/)') '  To use it, set listName="spex-path" in inp.xml.'
